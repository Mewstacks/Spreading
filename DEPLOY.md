...
# conectar sessão ML: /scrapers/  -> Login ML (headless no server é limitado, ver nota)
```
...
## Notas / limitações
- **Login ML no servidor**: `auth_stream` abre um browser visível; headless no Fly é limitado. Gere o `auth.json` localmente e suba pro volume, ou trate login ML remoto como tarefa futura.
